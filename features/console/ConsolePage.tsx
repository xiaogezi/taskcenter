...
type Session = { sessionId: string; title?: string; status?: string; taskCount?: number };
...
type PilotTask = { task_id: string; reported: boolean; qualification_evidence: string[]; prepare_turn: string; repeated_reads: number | "unknown"; recovery_cost_tokens: number | "unknown"; input_tokens: number | "unknown"; cached_input_tokens: number | "unknown"; rework_count: number | "unknown"; verification: string; review: string; authority_violations: number | "unknown" };
type PilotProject = { project_id: string; workspace: string; sessions: Array<{ session_id: string; title?: string }>; context_management: { state: string; experimental_mode: boolean | "unknown"; observed_at: string; applies_to: string }; latest_intent?: { action: string; status: string; message?: string } | null; trial: { completed: number; target: number; tasks: PilotTask[] } };
...
function Sessions() {
  ...
  const refreshStatus = useCallback(async () => { try { const status = await api<SessionData["status"]>("/session-status"); setState(current => current ? { ...current, status } : current); } catch (error) { setNotice(error instanceof Error ? error.message : "读取失败"); } }, []);
  useEffect(() => { const timer = window.setTimeout(load, 0); return () => window.clearTimeout(timer); }, [load]);
  useEffect(() => { const timer = window.setInterval(refreshStatus, 5000); return () => window.clearInterval(timer); }, [refreshStatus]);
...
function Settings() {
  ...
  const act = async (project: PilotProject, action: "enable" | "disable" | "refresh") => { try { const result = await api<{ intent: PilotProject["latest_intent"] }>("/context-management-pilots/intents", { method: "POST", headers: actionHeaders, body: JSON.stringify({ request_id: crypto.randomUUID(), project_id: project.project_id, workspace: project.workspace, action }) }); setPilots(current => ({ ...current, projects: (current?.projects || []).map(item => item.project_id === project.project_id && item.workspace === project.workspace ? { ...item, latest_intent: result.intent } : item) })); setNotice("试点 intent 已留档，等待主脑处理"); window.setTimeout(load, 300); } catch (error) { setNotice(error instanceof Error ? error.message : "试点操作失败"); } };
  const value = (item: string | number) => item === "unknown" ? "unknown" : item;
  return <><Header code="SERVICE SETTINGS" title="设置与边界" description="本机服务状态、数据来源、隐私边界与现有配置入口。" refresh={load} /><p>{notice}</p><div className="console-list"><article className="console-card"><h2>本地服务</h2><p>状态：{health?.ok ? "正常" : "未连接"} · 运行时间：{health?.control?.uptimeSeconds ?? "-"} 秒</p><button onClick={sync}>立即同步</button></article><article className="console-card"><h2>数据来源</h2><p>Dashboard：{health?.dashboard?.readable ? "可读" : "不可读"} · Ledger：{health?.ledger?.readable ? "可读" : "不可读"} · Watcher：{health?.watcher?.healthy ? "健康" : "需关注"}</p></article><article className="console-card"><h2>隐私与配置</h2><p>仅使用本机 Codex 会话索引与项目配置，不上传会话内容。</p><a href="/sessions">管理读取白名单</a> · <a href="/routing">调整模型策略</a></article></div><section className="console-pilots"><h2>Astra context_management 项目试点</h2><p>TaskCenter 只保存受审计 intent 并唤醒主脑，不直接修改外部项目或 ~/.codex。配置仅对新建 Astra 任务生效。</p><small>采集时间：{pilots?.generated_at || "unknown"}</small><div className="console-list">{(pilots?.projects || []).map(project => { const busy = ["pending", "processing"].includes(project.latest_intent?.status || ""); const session = project.sessions[0]; return <article className="console-card" key={`${project.project_id}:${project.workspace}`}><h2>{session?.title || "未命名会话"}</h2><p className="console-path">{session?.session_id || "无关联 Session"}</p><p className="console-path">{project.project_id} · {project.workspace}</p><p>experimental_mode：<strong>{project.context_management.state}</strong> · 采集：{project.context_management.observed_at || "unknown"} · 生效范围：仅新任务</p><p>Intent：{project.latest_intent ? `${project.latest_intent.action} / ${project.latest_intent.status}${project.latest_intent.message ? ` · ${project.latest_intent.message}` : ""}` : "unknown"}</p><p>启用后的连续 Astra 任务进度：{project.trial.completed}/{project.trial.target}</p>{project.trial.tasks.length ? project.trial.tasks.map(task => <div className="pilot-task" key={task.task_id}><strong>{task.task_id} · {task.reported ? "已上报" : "待上报"}</strong><p>资格证据 {task.qualification_evidence.join("；") || "unknown"} · prepare_turn {value(task.prepare_turn)} · 重复读取 {value(task.repeated_reads)} · 恢复成本 {value(task.recovery_cost_tokens)} · input/cachedInput {value(task.input_tokens)}/{value(task.cached_input_tokens)} · 返工 {value(task.rework_count)} · 验证 {value(task.verification)} · Review {value(task.review)} · 权威违规 {value(task.authority_violations)}</p></div>) : <p>自然长任务资格、prepare_turn、重复读取/恢复成本、input/cachedInput、返工、验证、Review、权威违规：unknown</p>}<button disabled={busy} onClick={() => act(project, "enable")}>启用</button><button disabled={busy} onClick={() => act(project, "disable")}>停用</button><button disabled={busy} onClick={() => act(project, "refresh")}>刷新</button><p><small>停用是当前任务的软停止，仍依赖既有历史；硬回退需关闭配置并新建 Astra 任务。</small></p></article>; })}</div>{!pilots?.projects?.length && <p>尚无带 project_id 与 workspace 的已登记项目。</p>}</section></>;
}
