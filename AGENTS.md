...
OCR independent review uses Luna as its primary reviewer and keeps reviewer independence across channel fallback. TaskCenter records the route, health state, retry timing, plus references or fingerprints for the same Subject, OCR Bundle, and rules. TaskCenter only returns and audits the recommendation; the Agent/OCR Skill must create the independent read-only Luna Session, load those exact inputs, and execute the review. If Luna is unavailable, OCR fails closed; Terra, Sol, Spark, or a normal implementation CLI Run must never be recorded as a successful independent OCR review substitute.
