...
OCR independent review keeps reviewer independence across model fallback. When Spark is known unavailable, TaskCenter recommends a Luna reviewer and records `fallback_from`, `fallback_reason`, `retry_after_at`, plus references or fingerprints for the same Subject, OCR Bundle, and rules. TaskCenter only returns and audits the recommendation; the Agent/OCR Skill must create the independent read-only Luna Session, load those exact inputs, and execute the review. Terra, Sol, or a normal implementation CLI Run must never be recorded as a successful independent OCR review substitute.
