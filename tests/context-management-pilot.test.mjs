...
test("按实际登记 workspace 读取项目级 experimental_mode，无法证明时为 unknown", async () => {
  ...
  const snapshot = pilotSnapshot({ registry, sessionTitles: [{ id: "a", title: "Astra 试点实现", updatedAt: "2026-09-07T23:00:00.000Z" }], now: "2026-09-08T00:00:00.000Z" });
  ...
  assert.deepEqual(snapshot.projects.find((item) => item.workspace === enabled).sessions[0], { session_id: "a", title: "Astra 试点实现", last_seen_at: "", updated_at: "2026-09-07T23:00:00.000Z" });
