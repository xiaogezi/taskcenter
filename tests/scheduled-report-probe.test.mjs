...
import { inspectManagedReport, inspectManagedReportContent, repairManagedReportHash } from "../scripts/scheduled-report-probe.mjs";
...
test("固定报告探针按 sha256-v1 排除 hash 行并 fail-closed", async () => {
  ...
  try {
    ...
    assert.equal(inspectManagedReportContent(await (await import("node:fs/promises")).readFile(report), report).valid, true);
    await writeFile(report, `<!-- AUTO-MANAGED-BEGIN -->${before}- managed_payload_sha256：\`${hash}\`\nchanged\n<!-- AUTO-MANAGED-END -->\n`);
    const invalid = inspectManagedReport(report);
    assert.equal(invalid.valid, false);
    assert.equal(invalid.reason, "managed_payload_hash_mismatch");
    assert.equal(inspectManagedReportContent(repairManagedReportHash(await (await import("node:fs/promises")).readFile(report))).valid, true);
  } finally {
