...
function RoutingPolicyPanel({ tasks, routingModels, modelRoles, usageReport, optionalAstraPolicy, onOptionalAstraPolicy }: { tasks: TaskRecord[]; routingModels: RoutingHealth[]; modelRoles?: { executor: ModelRole; reviewer: ModelRole }; usageReport?: UsageReport; optionalAstraPolicy?: OptionalAstraPolicy; onOptionalAstraPolicy: (policy: OptionalAstraPolicy) => void }) {
  ...
  const save = async (preset: OptionalAstraPolicy["preset"], threshold?: number) => { setSaving(true); setSaveMessage(""); try { const response = await fetch(`${controlServerUrl}/routing/optional-astra-policy`, { method: "POST", headers: { "Content-Type": "application/json", "X-TaskCenter-Action": "delegate" }, body: JSON.stringify({ preset, threshold_percent: threshold }) }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error || "策略保存失败"); onOptionalAstraPolicy(payload.policy); setSaveMessage("已保存"); } catch (error) { setSaveMessage(error instanceof Error ? error.message : "策略保存失败"); } finally { setSaving(false); } };

  return (
    <section id="routing-policy-control" className="routing-policy" aria-label="模型编排策略">
      ...
      <div className="routing-policy-grid">
        <div className="optional-policy-control"><strong>可选增强控制</strong>当前 {optionalAstraPolicy?.preset ?? "saving"} · 阈值 {optionalAstraPolicy?.threshold_percent ?? 0}%<small>真实额度 {primaryRateLimit?.used_percent ?? "未知"}%；{primaryRateLimit && primaryRateLimit.used_percent <= (optionalAstraPolicy?.threshold_percent ?? 0) ? "允许可选增强" : "关闭可选增强"}。最近修改：{optionalAstraPolicy?.updated_at ?? "默认"} / {optionalAstraPolicy?.updated_by ?? "default"}</small><div className="policy-presets"><button disabled={saving} aria-pressed={(optionalAstraPolicy?.preset ?? "saving") === "saving"} onClick={() => void save("saving")}>节省</button><button disabled={saving} aria-pressed={optionalAstraPolicy?.preset === "balanced"} onClick={() => void save("balanced")}>均衡</button><button disabled={saving} aria-pressed={optionalAstraPolicy?.preset === "quality"} onClick={() => void save("quality")}>质量优先</button></div><label className="policy-custom">自定义阈值 <input key={optionalAstraPolicy?.threshold_percent} aria-label="自定义 Astra 阈值" type="number" min="0" max="100" defaultValue={optionalAstraPolicy?.threshold_percent ?? 0}/><span>%</span><button disabled={saving} onClick={(event) => { const input = event.currentTarget.parentElement?.querySelector("input"); void save("custom", Number(input?.value)); }}>应用</button></label>{saveMessage && <small role="status">{saveMessage}</small>}</div>
