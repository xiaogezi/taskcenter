  ...
  return (
    <section className="routing-policy" aria-label="模型编排策略">
      ...
      <div className="routing-policy-grid">
        <p className="optional-policy-control"><strong>可选增强控制</strong>当前 {optionalAstraPolicy?.preset ?? "saving"} · 阈值 {optionalAstraPolicy?.threshold_percent ?? 0}%<small>真实额度 {primaryRateLimit?.used_percent ?? "未知"}%；{primaryRateLimit && primaryRateLimit.used_percent <= (optionalAstraPolicy?.threshold_percent ?? 0) ? "允许可选增强" : "关闭可选增强"}。最近修改：{optionalAstraPolicy?.updated_at ?? "默认"} / {optionalAstraPolicy?.updated_by ?? "default"}</small><div className="policy-presets"><button aria-pressed={(optionalAstraPolicy?.preset ?? "saving") === "saving"} onClick={() => void save("saving")}>节省</button><button aria-pressed={optionalAstraPolicy?.preset === "balanced"} onClick={() => void save("balanced")}>均衡</button><button aria-pressed={optionalAstraPolicy?.preset === "quality"} onClick={() => void save("quality")}>质量优先</button></div><label className="policy-custom">自定义阈值 <input aria-label="自定义 Astra 阈值" type="number" min="0" max="100" defaultValue={optionalAstraPolicy?.threshold_percent ?? 0}/><span>%</span><button onClick={(event) => { const input = event.currentTarget.parentElement?.querySelector("input"); void save("custom", Number(input?.value)); }}>应用</button></label></p>
